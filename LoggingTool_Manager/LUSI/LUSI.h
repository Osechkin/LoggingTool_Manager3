...
namespace LUSI
{
	...
	public:
		...
		double min;				// ћинимальное значение value
		double max;				// ћаксимальное значение value
		double value;			// «начение параметра (отображаемое и редактируемое значение)
		QString comment;		//  омментарии (отображаютс€ в виде ToolTip)
		QString units;			// ≈диницы измерени€
		QString formula;		// формула расчета примененного дл€ интервального программатора значени€ (не используетс€ на практике) 
		QString uitype;			// тип визуального элемента ввода значени€ параметра : 'SpinBox', 'CheckBox', 'ComboBox'
		bool ron;				// »змен€емый/неизмен€емый параметр
	...
	{		
		...
		QString datetime;					// дата и врем€ создани€ последовательности	
		QString description;				// описание последовательности

		QString js_script;					// программа последовательности на JavaScript

		QList<QByteVector> com_programs;	// список программ дл€ интервального программатора (обычно одна)
		QList<QByteVector> proc_programs;	// список пакетов обработки данных дл€ программы NMR_Tool на DSP
		QList<LUSI::Parameter*> param_list;	// список параметров последовательности
		QList<LUSI::Section*> section_list;		// список команд последовательности (в списке следуют строго по пор€дку)		
		QList<LUSI::Argument*> arg_list;	// список параметров-аргументов дл€ построени€ оси ќх дл€ вход€щих данных. ќб€зателен дл€ релаксационых спадов 
		QList<LUSI::Condition*> cond_list;	// список проверочных праметров, провер€ющих правильность задани€ параметров последовательности

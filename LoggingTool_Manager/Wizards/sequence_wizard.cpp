...
bool JSeqObject::partOf(QList<JSeqObject*> &jseq_list)
{	
	for (int i = 0; i < jseq_list.count(); i++)
	{
		if (jseq_list.at(i)->jseq_file == this->jseq_file) return true;
	}
	return false;
}
...
void SequenceWizard::showSeqParameters()
{
	...
	// —оздание раздела "Probes"
	QList<CSettings> title_settings_list_probes;
	CSettings title_settings_probe("label", tr("Probes"));
	...
	title_settings_list_probes.append(title_settings_probe);

	CTreeWidgetItem *c_title_probe = new CTreeWidgetItem(ui->treeWidget, 0, title_settings_list_probes);
	c_title_items.append(c_title_probe);
	c_title_probe->show();
	...
	for (int i = 0; i < param_count; i++)
	{
		...
		switch (param->type)
		{
		case Probe:		// если параметр - частота 
			{				
				//item_settings4.value = "";
				//item_settings_list_probe << item_settings1 << item_settings2 << item_settings3 << item_settings4;
				item_settings_list_probe << item_settings1 << item_settings3 << item_settings4;
				CTreeWidgetItem *c_item = new CTreeWidgetItem(ui->treeWidget, c_title_probe->getQSubTreeWidgetItem(), item_settings_list_probe);
		...
		case Frequency:		// если параметр - частота 
			{				
				...
				item_settings_list_freq << item_settings1 << item_settings3 << item_settings4;
		...
		case RF_Pulse:		// если параметр - радиочастотный импульс 
			{
				...
				item_settings_list_rfpulses << item_settings1 << item_settings3 << item_settings4;
		...
		case Phase:		// если параметр - фаза 
			{
				...
				item_settings_list_phases << item_settings1 << item_settings3 << item_settings4;
...
bool SequenceWizard::changeCurrentSequence(const QString &text)
{
	...
	if (file.open(QIODevice::ReadOnly | QIODevice::Text))
	{
		...
		jseq_file = file_name;
		emit new_sequence_file(QFileInfo(file_name));
	}
